...
// =========================================================================
// SOURCE ENGINE — 8 confirmed-working TMDB embed providers
// Features: multi-language, multi-server internals, sub/dub
// No sandbox. Click-to-play overlay handles sound (browser autoplay policy).
// =========================================================================
...
const SERVERS = {
    default: [
        {
            name: 'VidSrc.cc',
            desc: 'Premium sources · HD streams',
            icon: 'fas fa-gem',
        ...
        {
            name: 'VidLink Pro',
            desc: 'Multi-server · Sub/Dub · Multi-lang',
            icon: 'fas fa-satellite-dish',
            build: (t, id, s, e) => t === 'movie'
                ? `https://vidlink.pro/movie/${id}?primaryColor=ff2d55`
                : `https://vidlink.pro/tv/${id}/${s}/${e}?primaryColor=ff2d55`
        },
        {
            name: 'VidSrc.me',
            desc: 'Reliable mirror · Fast CDN',
            icon: 'fas fa-bolt',
            build: (t, id, s, e) => t === 'movie'
        ...
        {
            name: '2Embed',
            desc: 'Reliable fallback · Wide support',
            icon: 'fas fa-film',
            build: (t, id, s, e) => t === 'movie'
    ...
    anime: [
        ...
        {
            name: 'VidLink Pro',
            desc: 'Sub/Dub · Skip Intro · Multi-lang',
            icon: 'fas fa-satellite-dish',
            build: (t, id, s, e) => t === 'movie'
    ...
    kdrama: [
        {
            name: 'VidSrc.cc',
            desc: 'PRIMARY · Korean drama · HD · Fast',
            icon: 'fas fa-gem',
            build: (t, id, s, e) => t === 'movie'
                ? `https://vidsrc.cc/v2/embed/movie/${id}`
                : `https://vidsrc.cc/v2/embed/tv/${id}/${s}/${e}`
        ...
        {
            name: 'VidSrc.me',
            desc: 'Reliable · Korean sub support',
            icon: 'fas fa-bolt',
            build: (t, id, s, e) => t === 'movie'
                ? `https://vidsrc.me/embed/movie?tmdb=${id}`
                : `https://vidsrc.me/embed/tv?tmdb=${id}&season=${s}&episode=${e}`
        ...
        {
            name: 'VidLink Pro',
            desc: 'K-Drama · Multi-lang subs',
            icon: 'fas fa-satellite-dish',
            build: (t, id, s, e) => t === 'movie'
                ? `https://vidlink.pro/movie/${id}?primaryColor=ec4899`
                : `https://vidlink.pro/tv/${id}/${s}/${e}?primaryColor=ec4899`
        },
        {
            name: '2Embed',
            desc: 'Reliable fallback · Wide support',
        ...
        {
            name: 'AutoEmbed',
            desc: 'Auto-switch · Korean support',
            icon: 'fas fa-magic',
            build: (t, id, s, e) => t === 'movie'
                ? `https://autoembed.co/movie/tmdb/${id}`
                : `https://autoembed.co/tv/tmdb/${id}-${s}-${e}`
    ...
    turkish: [
        {
            name: 'VidLink Pro',
            desc: 'Turkish series · Multi-lang',
            icon: 'fas fa-satellite-dish',
            build: (t, id, s, e) => t === 'movie'
                ? `https://vidlink.pro/movie/${id}?primaryColor=ff6b35`
                : `https://vidlink.pro/tv/${id}/${s}/${e}?primaryColor=ff6b35`
        },
        {
            name: 'VidSrc.cc',
            desc: 'Fast CDN · Multi-lang',
            icon: 'fas fa-gem',
            build: (t, id, s, e) => t === 'movie'
                ? `https://vidsrc.cc/v2/embed/movie/${id}`
                : `https://vidsrc.cc/v2/embed/tv/${id}/${s}/${e}`
        ...
        {
            name: 'VidSrc.me',
            desc: 'Reliable · Turkish sub support',
            icon: 'fas fa-bolt',
            build: (t, id, s, e) => t === 'movie'
                ? `https://vidsrc.me/embed/movie?tmdb=${id}`
                : `https://vidsrc.me/embed/tv?tmdb=${id}&season=${s}&episode=${e}`
        ...
        {
            name: 'AutoEmbed',
            desc: 'Auto-switch · Turkish support',
            icon: 'fas fa-magic',
            build: (t, id, s, e) => t === 'movie'
                ? `https://autoembed.co/movie/tmdb/${id}`
                : `https://autoembed.co/tv/tmdb/${id}-${s}-${e}`
    ...
    asian: [
        {
            name: 'VidSrc.cc',
            desc: 'PRIMARY · Asian Drama · HD · Fast',
            icon: 'fas fa-gem',
            build: (t, id, s, e) => t === 'movie'
                ? `https://vidsrc.cc/v2/embed/movie/${id}`
                : `https://vidsrc.cc/v2/embed/tv/${id}/${s}/${e}`
        ...
        {
            name: 'VidSrc.me',
            desc: 'Reliable · Asian sub support',
            icon: 'fas fa-bolt',
            build: (t, id, s, e) => t === 'movie'
                ? `https://vidsrc.me/embed/movie?tmdb=${id}`
                : `https://vidsrc.me/embed/tv?tmdb=${id}&season=${s}&episode=${e}`
        ...
        {
            name: 'VidLink Pro',
            desc: 'Multi-lang · High Speed',
            icon: 'fas fa-satellite-dish',
            build: (t, id, s, e) => t === 'movie'
                ? `https://vidlink.pro/movie/${id}?primaryColor=3b82f6`
                : `https://vidlink.pro/tv/${id}/${s}/${e}?primaryColor=3b82f6`
        },
        {
            name: '2Embed',
            desc: 'Reliable fallback · Wide support',
...
function buildServers() {
    ...
    container.innerHTML = activeServers.map((srv, idx) => {
        const isActive = idx === state.serverIdx;
        const activeClass = isActive
            ? 'bg-pulse text-white border-pulse shadow-lg shadow-pulse/30 scale-[1.04]'
            : 'bg-white/5 border-white/10 text-gray-400 hover:text-white hover:bg-white/10 hover:scale-[1.04]';
        return `
        <button onclick="switchServer(${idx})"
            title="${srv.desc || srv.name}"
            class="srv-btn relative flex flex-col items-start gap-1 px-4 py-3 rounded-xl border text-left transition-all duration-200 ${activeClass}">
            <div class="flex items-center gap-2">
                <i class="${srv.icon || 'fas fa-play-circle'} text-xs"></i>
                <span class="text-[10px] font-black uppercase tracking-widest">${srv.name}</span>
            </div>
            <span class="text-[8px] font-bold tracking-wider opacity-60 leading-tight">${srv.desc || ''}</span>
        </button>`;
    }).join('');
}
function switchServer(idx) {
    stopAutoSwitchWatchdog(); // cancel any pending auto-switch
    state.serverIdx = idx;
    updateStream(); // updateStream calls buildServers + loadStream + starts watchdog
}
...
// =========================================================================
// AUTO SERVER SWITCH WATCHDOG
// If iframe doesn't fire a load event within 12 seconds, silently try the
// next server. Cycles through all 8 servers before giving up.
// =========================================================================
let autoSwitchTimer = null;
let autoSwitchAttempts = 0;
function startAutoSwitchWatchdog() {
    clearTimeout(autoSwitchTimer);
    const serverList = SERVERS[state.category] || SERVERS['default'];
    const totalServers = serverList.length;
    // Show loader hint in server button
    const activeBtn = document.querySelector('.srv-btn.scale-105') ||
                      document.querySelectorAll('.srv-btn')[state.serverIdx];
    if (activeBtn) {
        activeBtn.style.opacity = '0.7';
    }
    autoSwitchTimer = setTimeout(() => {
        // Only auto-switch if we haven't tried all servers
        if (autoSwitchAttempts < totalServers - 1) {
            autoSwitchAttempts++;
            const nextIdx = (state.serverIdx + 1) % totalServers;
            // Hide overlay before switching silently
            const overlay = document.getElementById('playUnmuteOverlay');
            if (overlay) overlay.style.display = 'none';
            playerShowNotification(`Auto-switching to ${serverList[nextIdx].name}...`);
            state.serverIdx = nextIdx;
            buildServers();
            loadStream(); // load without resetting watchdog counter
        } else {
            autoSwitchAttempts = 0;
            playerShowNotification('All sources tried. Select a server manually.');
        }
    }, 12000);
}
function stopAutoSwitchWatchdog() {
    clearTimeout(autoSwitchTimer);
    autoSwitchAttempts = 0;
    // Restore button opacity
    document.querySelectorAll('.srv-btn').forEach(b => b.style.opacity = '');
}
// Inner load — builds URL and sets iframe src only (no watchdog reset)
// Tracks the pending URL so dismissPlayOverlay can re-fire it with sound
let _pendingStreamUrl = '';
function loadStream() {
    const serverList = SERVERS[state.category] || SERVERS['default'];
    ...
    iframe.src = url;
    // Stop watchdog when iframe fires load (content responded)
    iframe.onload = () => stopAutoSwitchWatchdog();
    // Show the click-to-unmute overlay ONLY if we don't already have user activation
    // Browsers allow audio if the iframe is set/reloaded within a user gesture.
    const overlay = document.getElementById('playUnmuteOverlay');
    if (overlay) {
...
// Called when user taps the "Click to Play with Sound" overlay.
// The tap IS the user gesture the browser needs to allow audio.
window.dismissPlayOverlay = function () {
    const overlay = document.getElementById('playUnmuteOverlay');
    if (overlay) overlay.style.display = 'none';
    // Re-fire the iframe src NOW (directly inside the user gesture callback)
    // This tells the browser to allow audio for this navigation.
    const iframe = document.getElementById('neuralIframe');
    if (iframe && _pendingStreamUrl) {
        iframe.src = _pendingStreamUrl;
    }
};
function updateStream() {
    autoSwitchAttempts = 0; // reset on deliberate server change or episode change
    buildServers(); // refresh active highlight
...
window.toggleIframeSandbox = function () {};
let searchTimeout;
